...
import { fetchDemoAnalytics } from "@/lib/demo-utils"
...
export default function DemoAnalyticsPage() {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      setIsLoading(true)
      try {
        const data = await fetchDemoAnalytics()
        setAnalytics(data)
      } catch (error) {
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-yellow-100 dark:bg-yellow-900 border border-yellow-300 dark:border-yellow-700 rounded-md p-4 mb-8">
          <p className="text-yellow-800 dark:text-yellow-200 text-center text-sm">
            This is demo data. In the full version, your analytics will be personalized based on your actual usage.
          </p>
        </div>
