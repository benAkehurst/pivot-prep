...
import { toggleDemoCompleted, saveDemoEvaluation } from "@/lib/demo-utils"
...
export default function DemoFlashcard({
  ...
  const handleToggleCompleted = async (e: React.MouseEvent) => {
    e.stopPropagation()
    try {
      const newStatus = await toggleDemoCompleted(id)
  ...
  const handleRecordingComplete = async (audioBlob: Blob) => {
    try {
      ...
      // Step 3: Save evaluation to local storage
      await saveDemoEvaluation(id, transcriptionData.text, evaluationData)
    } catch (error) {
